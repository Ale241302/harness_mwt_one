---
name: mwt-superadmin-nodos-editar
description: "Rol Super Admin · módulo Nodos (nodos) · permiso editar (actualizar/modificar). Úsala cuando el usuario quiera editar, actualizar, corregir o cambiar algo en nodos. Herramientas MCP: nodo_editar, artefacto_editar, artefacto_publicar. Lee el contrato en _contratos/nodos.md antes de actuar."
role: superadmin
module: nodos
action: update
---

# mwt-superadmin-nodos-editar

> Skill MCP enfocado: **Super Admin** · **Nodos** · permiso **editar** (actualizar/modificar).

## Propósito
Operar el módulo **Nodos** (`nodos`) con la acción **editar** (actualizar/modificar) usando SOLO
...
## Herramientas MCP para esta acción
  - `nodo_editar` — Edita un nodo (PATCH parcial).
  - `artefacto_editar` — `cambios` admite un subconjunto de: `data` (valores por field.id),
  - `artefacto_publicar` — Con `publicado=true` el artefacto se vuelve visible para clientes (roles

## Firmas (cómo invocar)
  - `nodo_editar` → `def nodo_editar(nodo_id: str, cambios: dict) -> Any:`
  - `artefacto_editar` → `def artefacto_editar(nodo_id: str, artifact_id: str, cambios: dict) -> Any:`
  - `artefacto_publicar` → `def artefacto_publicar(nodo_id: str, artifact_id: str, publicado: bool = True) -> Any:`
...
- El alcance de este skill es **editar** sobre **Nodos**; para otra operación activá el SKILL.md correspondiente.
