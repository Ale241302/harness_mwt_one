...
description: "Rol Super Admin · módulo Expedientes (expedientes) · permiso eliminar (eliminar/borrar). Úsala cuando el usuario quiera eliminar o borrar algo de expedientes. Herramientas MCP: expediente_eliminar, documento_eliminar. Lee el contrato en _contratos/expedientes.md antes de actuar."
role: superadmin
module: expedientes
action: delete
---

# mwt-superadmin-expedientes-eliminar

> Skill MCP enfocado: **Super Admin** · **Expedientes** · permiso **eliminar** (eliminar/borrar).

## Propósito
Operar el módulo **Expedientes** (`expedientes`) con la acción **eliminar** (eliminar/borrar) usando SOLO
...
## Herramientas MCP para esta acción
  - `expediente_eliminar` — expediente activo de su OC, la OC también se borra. Úsalo para expedientes FANTASMA
  - `documento_eliminar`

## Firmas (cómo invocar)
  - `expediente_eliminar` → `def expediente_eliminar(expediente_id: str) -> Any:`
  - `documento_eliminar` → `def documento_eliminar(documento_id: str) -> Any:`
...
- El alcance de este skill es **eliminar** sobre **Expedientes**; para otra operación activá el SKILL.md correspondiente.
