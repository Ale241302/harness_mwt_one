---
name: mwt-admin-storage-subir-documento
description: "Rol Admin (CEO) · módulo Storage (storage) · permiso subir-documento (subir un archivo/documento). Úsala cuando el usuario quiera subir o adjuntar un documento a storage. Herramientas MCP: ninguna tool directa en esta acción. Lee el contrato en _contratos/storage.md antes de actuar."
role: admin
module: storage
action: upload_doc
---

# mwt-admin-storage-subir-documento

> Skill MCP enfocado: **Admin (CEO)** · **Storage** · permiso **subir-documento** (subir un archivo/documento).

## Propósito
Operar el módulo **Storage** (`storage`) con la acción **subir-documento** (subir un archivo/documento) usando SOLO
...
- Para subir un archivo a MinIO usá `storage_subir_archivo` (`storage.create`).
...
- El alcance de este skill es **subir-documento** sobre **Storage**; para otra operación activá el SKILL.md correspondiente.
