---
name: mwt-viewer-storage-ver-documento
description: "Rol Viewer (solo lectura) · módulo Storage (storage) · permiso ver-documento (ver/listar documentos). Úsala cuando el usuario quiera ver o listar los documentos de storage. Herramientas MCP: ninguna tool directa en esta acción. Lee el contrato en _contratos/storage.md antes de actuar."
role: viewer
module: storage
action: view_doc
---

# mwt-viewer-storage-ver-documento

> Skill MCP enfocado: **Viewer (solo lectura)** · **Storage** · permiso **ver-documento** (ver/listar documentos).

## Propósito
Operar el módulo **Storage** (`storage`) con la acción **ver-documento** (ver/listar documentos) usando SOLO
...
- No hay listado de 'documentos' en storage: los artefactos se ven con `nodo_artefactos_listar` o `inventario_artefactos_expediente`.
...
- El alcance de este skill es **ver-documento** sobre **Storage**; para otra operación activá el SKILL.md correspondiente.
