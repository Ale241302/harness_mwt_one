---
name: mwt-manager-inventario-editar
description: "Rol Manager · módulo Inventario (inventario) · permiso editar (actualizar/modificar). Úsala cuando el usuario quiera editar, actualizar, corregir o cambiar algo en inventario. Herramientas MCP: inventario_transferir_asignaciones. Lee el contrato en _contratos/inventario.md antes de actuar."
role: manager
module: inventario
action: update
---

# mwt-manager-inventario-editar

> Skill MCP enfocado: **Manager** · **Inventario** · permiso **editar** (actualizar/modificar).

## Propósito
Operar el módulo **Inventario** (`inventario`) con la acción **editar** (actualizar/modificar) usando SOLO
...
## Herramientas MCP para esta acción
  - `inventario_transferir_asignaciones`

## Firmas (cómo invocar)
  - `inventario_transferir_asignaciones` → `def inventario_transferir_asignaciones(origin_nodo_id: str, destination_nodo_id: str, items: list, transferencia_id: str | None = None) -> Any:`
...
- El alcance de este skill es **editar** sobre **Inventario**; para otra operación activá el SKILL.md correspondiente.
