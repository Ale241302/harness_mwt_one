...
- The seed opens with the message uid and a guardrail: it labels the body context rather than an instruction, ends by telling the model to stay out of the mailbox and the business MCP until the user asks, and records the uid so a later request for the attached document is served without a search. The gateway workspace instructions restate the rule and name the business-intent criterion for the MCP.
