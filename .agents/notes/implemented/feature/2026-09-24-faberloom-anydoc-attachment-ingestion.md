...
- A new `packages/faberloom/view/src/documents.ts` resolves the converter through the Node module graph (`createRequire` on `@firecrawl/anydoc/package.json`, then its `bin` entry) and spawns its `cli.js` launcher through the harness subprocess provider (`ctx.get('subprocess')`), never through `node:child_process`. A JavaScript launcher runs under the current Node so Windows needs no exec bit.
...
- The view package gains the runtime dependency `@firecrawl/anydoc` and the peer `@deepseek-ai/dsh-subprocess`, with a project reference from its `tsconfig.json`; `pnpm-lock.yaml` moves.
...
`packages/faberloom/view/tests/documents.spec.ts` covers format mapping, the no-spawn paths (empty batch, unsupported attachment), a successful conversion with its argv and stdio, hosted-OCR argv, a non-zero exit, and one failing spawn among two documents. A real invocation of `node <anydoc>/cli.js <file> --format csv` was run once on Windows to confirm the binary converts.
