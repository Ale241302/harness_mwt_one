...
| `ctx.faberloomMemory` | `core` | `learning` | - | - | - | 拥有版本化教学及其记录来源、范围与撤销，以及按任务与情境的绩效证据；它从不授予权限。 |
| `ctx.faberloomAccess` | `core` | `access` | - | - | - | 拥有按用户身份绑定、连接引用，以及按动作范围的自主授权；这些授权在每次外部效应前校验，并可随时撤销。 |
| `ctx.faberloomBackup` | `core` | `backup` | - | - | - | 拥有产品各领域的导出、恢复、完整性清单与迁移，只恢复当前权限仍允许的内容。 |
| `ctx.faberloomConnections` | `core` | `connections` | - | - | - | 拥有 owner 输入的 IMAP 邮箱与知识备份设置，按身份保存并以真实探测校验；从不依赖 MWT.ONE MCP。 |
| `ctx.faberloomHandlers` | `core` | `handlers` | - | - | - | 实现例程步骤声明的每个 handler 名称，使引擎能够激活例程；wait 处理器让运行停在等待状态，直到该步骤声明的事件到来。 |
| `ctx.faberloomDefaults` | `core` | `defaults` | - | - | - | 按 owner 播种一次计划派生的代理与例程，只分配 owner 角色随附的技能；播种出的例程在 owner 激活前保持草稿状态。 |
| `ctx.faberloomInbound` | `core` | `inbound` | - | - | - | 轮询 owner 配置的 IMAP 邮箱并把新邮件转换为例程事件，不标记也不移动其邮件；幂等由引擎的 Message-ID 与 UID 键保证。 |
| `ctx.faberloomMcpServer` | `core` | `mcp-server` | - | - | - | 在每个 owner 的 socket 上以 MCP Streamable HTTP 提供 JSON-RPC，用 owner 创建的 bearer 令牌认证每个客户端，并通过产品服务完成每次调用。 |
| `ctx.faberloomView` | `core` | `view` | - | - | - | 为 FaberLoom 面板把 owner 范围内的空间、代理、看板与例程投影为纯 JSON 行；不执行写入，其身份来自部署配置。 |
